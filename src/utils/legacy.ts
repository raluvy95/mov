...
export async function legacyRank(
    msg: Message<any>,
    rank: ILevelDB,
    user: User,
) {
    const reqXP = formulaXP(rank.level);
    const settingsDB = (await client.database.settings.get<ISettingsDB>(
        msg.guildID!,
    ))!;
    const nextUp = settingsDB.modules!.level!.roleRewards?.find(
        (m) => rank.level < m.level,
    );
    ...
    const e = new MovEmbed()
        ...
        .addField(
            "Progress Bar",
            `[${rank.xp.toLocaleString()}/${reqXP.toLocaleString()}] ${progressBar(
                rank.xp,
                reqXP,
            )} (${percent}%)`,
        )
...
export async function legacyLeaderboard(
