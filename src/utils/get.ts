...
export async function getUser(msg: Message<any>, name: string): Promise<User | undefined> {
    ...
    const cached = client.users.find(m =>
        m.username.includes(name) ||
        m.username == name)

    if (!cached) {
        const m = await client.searchGuildMembers(process.env.SERVER_ID!, name, 1)
        if (m.length < 1) {
            return undefined
        } else {
            return m[0].user
