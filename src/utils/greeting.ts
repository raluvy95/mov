...
export async function greeting(type: "w" | "g", guild: Guild, member: Member | MemberPartial) {
    if (guild.id != process.env.SERVER_ID!) return;

    const greeting = await client.database.settings.get<ISettingsDB>(guild.id)
    if (!greeting) return;
    const w = type == "w" ? greeting.modules.welcome : greeting.modules.goodbye
    ...
    if (!channel) { console.warn("Cannot find a channel to send."); return }

    let content = !w.message ? "Hello {mention} and welcome to the server! We now have {memberCount}!"
        : w.message
            .replace("{server}", guild.name)
            .replace("{memberCount}", guild.memberCount.toString())
            .replace("{name}", member.user.username + "#" + member.user.discriminator)
