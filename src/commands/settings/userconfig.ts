...
async function generator(msg: Message, args: string[]) {
    ...
    if (args.length > 0) {
        ...
        switch (type) {
            ...
            case "noMentionOnLevelUP":
                if (!["true", "false"].includes(value.toLowerCase())) {
                    client.createMessage(
                        msg.channel.id,
                        `Type "true" or "false", not "${value.toLowerCase()}"`,
                    );
                    return;
                }
            ...
            case "prefix":
                ...
                if (value.length >= 32) {
                    client.createMessage(
                        msg.channel.id,
                        "Your prefix has reached the limit of character! (32 max)",
                    );
                    return;
                }
                break;
        ...
        client.createMessage(
            msg.channel.id,
            `Successfully changed for your ${type}!`,
        );
        return;
    }
    const listAlias = uSettings.aliases
        .map(
            (m) =>
                `**${m.commandTarget}** => (${m.alias
                    .map((n) => `\`${n}\``)
                    .join(", ")})`,
        )
        .join("\n");
    const e = new MovEmbed()
        ...
        .setDesc(
            `Use \`${msg.prefix}help userconfig\` to check out how to edit and examples!`,
        )
        .addField(
            "Prefix [`prefix`]",
            uSettings.prefix || "No user prefix",
            true,
        )
        .addField("Color Accent [`colorAccent`]", uSettings.colorAccent, true)
        .addField(
            "No mention on level up message [`noMentionOnLevelUP`]",
            !uSettings.noMentionOnLevelUP ? "No" : "Yes",
            true,
        )
        .addField(
            "Use legacy rank [`useLegacyRank`]",
            !uSettings.useLegacyRank ? "No" : "Yes",
            true,
        )
        .addField(
            "Background URL [`customBackgroundURL`]",
            uSettings.customBackgroundURL !== "color"
                ? `[click to view](${uSettings.customBackgroundURL})`
                : "color",
        )
        .addField(
            "Aliases",
            uSettings.aliases?.length > 0
                ? listAlias.length >= 1000
                    ? listAlias.slice(0, 950) + "... `$aliases` to show more"
                    : listAlias
                : "No user alias",
        );
...
class UserSettings extends MovCommand {
    constructor() {
        super("userconfig", generator, {
            ...
            description:
                "User settings - configure user alias and preferencies\n" +
