...
async function generator(msg: Message, args: string[]) {
    ...
    if (args.length >= 1) {
            ...
            if (modulee === "json") {
                ...
                e.setTitle(`View detailed for ${modulee} in JSON`)
                    ...
                    .addField(
                        ...
                        `Use ${msg.prefix}examples to view all`,
                    );
        ...
        if (!key) {
            client.createMessage(
                msg.channel.id,
                "What key do you want to change to?",
            );
            return;
        } else if (!value) {
            client.createMessage(
                msg.channel.id,
                "What value for key do you want to change?",
            );
            return;
        }
        switch (subcommand.toLowerCase()) {
            case "add":
                if (
                    ...
                    client.createMessage(
                        msg.channel.id,
                        "The key you're looking for is not an array!",
                    );
                    return;
                }
                try {
                    ...
                    await client.database.settings.set<ISettingsDB>(
                        msg.guildID!,
                        settings,
                    );
                    ...
                    client.createMessage(
                        msg.channel.id,
                        `There's something went wrong: ${e}`,
                    );
                    return;
                }
                break;
                ...
                try {
                    try {
                        await client.database.settings.set(
                            `${msg.guildID!}.modules.${modulee}.${key}`,
                            JSON.parse(value),
                        );
                    } catch {
                        await client.database.settings.set(
