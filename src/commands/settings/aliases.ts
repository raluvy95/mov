...
async function generator(msg: Message, args: string[]) {
    ...
    if (!uSettings) {
        uSettings = await client.database.user.set<IUserDB>(
            msg.author.id,
    ...
    const allCommandsNameAndAliases = Object.values(client.commands)
        .flatMap((m) => m.aliases)
        .concat(Object.keys(client.commands))
        .concat(uSettings.aliases.flatMap((m) => m.alias));
    if (args.length < 1 || !isNaN(Number(args[0]))) {
        ...
        try {
            const e = new MovEmbed()
                ...
                .setDesc(
                    `If you want to modify your aliases, use \`${msg.prefix}aliases add <alias name> <target>\` or \`${msg.prefix}aliases remove <alias name>\`\nYou cannot have user alias if it conflicts with command name, its build-in aliases AND your user alias`,
                );
            if (uSettings.aliases.length < 1) {
                e.addField(
                    "No aliases?",
                    "you don't have any user aliases set",
                );
            } else {
                for (const alias of uSettings.aliases.slice(
                    (page - 1) * 20,
                    page * 20,
                )) {
                    e.addField(
                        `Target: ${alias.commandTarget}`,
                        alias.alias.map((m) => `\`${m}\``).join(", "),
                    );
                }
            ...
            client.createMessage(
                msg.channel.id,
                "There was an error viewing your aliases. Please contact the developer for this!",
            );
            return;
        }
    }
    ...
    switch (action.toLowerCase()) {
        ...
        case "create": {
            if (allCommandsNameAndAliases.includes(name)) {
                client.createMessage(
                    msg.channel.id,
                    "Your alias is conflict with existing command name, alias or your other aliases. Please try a different one",
                );
                return;
            }
            if (!target) {
                client.createMessage(
                    msg.channel.id,
                    "What's that alias for? (Missing target command)",
                );
                return;
            }
            if (name.length >= 150) {
            ...
            if (!cmd) {
                client.createMessage(
                    msg.channel.id,
                    "That target command is not found.",
                );
                return;
            }
            const existingCmd = uSettings.aliases.find(
                (m) => m.commandTarget === cmd.label,
            );
            if (!existingCmd) {
                ...
                uSettings.aliases.splice(
                    uSettings.aliases.findIndex(
                        (m) => m.commandTarget === cmd.label,
                    ),
                    1,
                );
        ...
        case "remove":
        case "rm": {
            const existingAlias = uSettings.aliases.find((m) =>
                m.alias.includes(name),
            );
            if (!existingAlias) {
                client.createMessage(
                    msg.channel.id,
                    "That alias doesn't exist. You probably already removed it",
                );
                return;
            }
            existingAlias.alias.splice(
                existingAlias.alias.findIndex((m) => m === name),
                1,
            );
            uSettings.aliases.splice(
                uSettings.aliases.findIndex(
                    (m) => m.commandTarget === existingAlias.commandTarget,
                ),
                1,
            );
