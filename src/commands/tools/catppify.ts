...
async function generator(msg: Message, args: string[]) {
    ...
    if (ext?.toLowerCase() === "webp") {
        client.createMessage(
            msg.channel.id,
            "Webp is not supported at this moment",
        );
        return;
    }
    ...
    if (!palette) {
        ...
        if (!avaliable_palette.includes(palette)) {
            client.createMessage(
                msg.channel.id,
                `Invalid palette. Use one of the following avaliable palettes: ${avaliable_palette.join(
                    ", ",
                )}`,
            );
            return;
    ...
    if (!noise) {
        ...
        if (!avaliable_noise.includes(noise)) {
            client.createMessage(
                msg.channel.id,
                `Invalid palette. Use one of the following avaliable noises: ${avaliable_noise.join(
                    ", ",
                )}`,
            );
            return;
    ...
    client.createMessage(msg.channel.id, "Ok, please be patient while processing. May take longer depending on image size.").then(async () => {
        ...
        client.createMessage(
            msg.channel.id,
            `Here you go (Took ${ms(diff.toMillis())})`,
            {
                file: generated,
                name: "generated.png",
            },
