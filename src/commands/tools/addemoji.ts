...
async function generator(msg: Message, args: string[]) {
    ...
    if (isEmote(args)) {
        ...
        client.createGuildEmoji(msg.guildID!, {
            name: name,
            image: await urlToDataURI(url)
        }).then(e => {
            client.createMessage(msg.channel.id, `I added <${e.animated ? 'a' : ''}:${e.name}:${e.id}> to this guild!`);
        }).catch(e => client.createMessage(msg.channel.id, `It looks like I got an error!\n${e}`))
    } else if (isURL(args)) {
        if (!args[1]) {
            client.createMessage(msg.channel.id,
        ...
        client.createGuildEmoji(msg.guildID!, {
            ...
            client.createMessage(msg.channel.id, `I added <${e.animated ? 'a' : ''}:${e.name}:${e.id}> to this guild!`);
        }).catch(e => client.createMessage(msg.channel.id, `I got an error!\n${e}`))
    } else {
        client.createMessage(msg.channel.id, "It looks like your result is invalid")
        return
    }
}
