...
async function generator(msg: Message, args: string[]) {
        ...
        if (command !== undefined) {
            ...
            const perm = () => {
                if (
                    ...
                    Object.keys(command.requirements.permissions).length < 1
                )
                    return "No permissions is required";
                let result = "";
            ...
            const usagestat = await client.database.cmdStat.get<number>(
                command.label,
            );
            emb.setTitle(`Command: ${command.label}`)
                ...
                .addFields([
                    ...
                    {
                        name: "Cooldown",
                        value: `${(command.cooldown / 1000).toString()}s`,
                        inline: true,
                    },
                    {
                        name: "Usage",
                        value: `${prefix}${command.label} ${command.usage}`,
                        inline: true,
                    },
                    {
                        name: "DM only",
                        value: bool(command.dmOnly),
                        inline: true,
                    },
                    {
                        name: "Server only",
                        value: bool(command.guildOnly),
                        inline: true,
                    },
                    {
                        name: "Usage count",
    ...
    for (const mod of modules) {
        ...
        emb.addField(
            mod.toUpperCase(),
            registeredCommand.map((m) => `\`${m}\``).join(", "),
        );
        registeredCommand = [];
    }
