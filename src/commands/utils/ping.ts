...
function generator(msg: Message, _args: string[]) {
    client.createMessage(msg.channel.id, "Pinging...").then(async m => {
        ...
        await m.edit(`Pong! Discord API: \`${client.shards.get(0)?.latency == Infinity ? '???' : client.shards.get(0)?.latency}ms\` Actual Response: \`${diff}ms\``)
    })
}

class Ping extends MovCommand {
    constructor() {
        super("ping", generator, { cooldown: 10 })
    }
}
