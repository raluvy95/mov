...
export class MovEmbed implements Embed {
    ...
    setAuthor(name: string, iconURL?: string, url?: string) {
        if (name.length > EMBED.AUTHOR_NAME)
            throw new Error(
                `Reached the limit length allowed (${EMBED.AUTHOR_NAME}`,
            );
        this.author = {
            name,
            icon_url: iconURL,
    ...
    addFields(fields: EmbedField[]) {
        ...
        if (this.fields.length > EMBED.FIELDS)
            throw new Error(
                `Cannot have more than ${EMBED.FIELDS} fields in size`,
            );
    ...
    addField(name: string, value: string, inline?: boolean) {
        if (name.length > EMBED.FIELD_NAME)
            throw new Error(
                `Reached the limit length allowed (${EMBED.FIELD_NAME}`,
            );
        if (value.length > EMBED.FIELD_VALUE)
            throw new Error(
                `Reached the limit length allowed (${EMBED.FIELD_VALUE}`,
            );
