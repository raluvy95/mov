...
export default new MovPlugin("bump", {
    event: "messageCreate",
    async run(msg) {
        const serverPref = (await client.database.settings.get<ISettingsDB>(
            msg.guildID!,
        ))!;
        if (
            serverPref.modules.bump.enable &&
            msg.interaction?.name === "bump" &&
            msg.author.id === "302050872383242240"
        ) {
            let target = "";
            const opt: AdvancedMessageContent = {};
            if (
                !serverPref.modules.bump.roleID ||
                serverPref.modules.bump.roleID === "0"
            ) {
            ...
            if (serverPref.modules.bump.bumpRewards) {
                const level = await client.database.settings.get<ISettingsDB>(
                    msg.guildID!,
                );
                if (level?.modules.level.enable) {
                    const reward = 100 * level.modules.level.multiplyXP!;

                    let lvl = await client.database.level.get<ILevelDB>(
                        msg.interaction.user.id,
                    );

                    if (!lvl) {
                        lvl = await client.database.level.set<ILevelDB>(
                            msg.interaction.user.id,
                            EMPTY_LEVEL,
                        );
                    }
                    ...
                    await client.database.level.set(
                        msg.interaction.user.id,
                        lvl,
                    );
                }
            ...
            setTimeout(() => {
                client.createMessage(msg.channel.id, {
                    content: `Hey <@${target}>, reminder to \`/bump\` again!`,
                    ...opt,
                });
            }, 7200000);
        }
