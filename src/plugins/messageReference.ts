...
export default new MovPlugin("messageReference", {
    event: "messageCreate",
    async run(msg) {
        const matched = msg.content.match(/http(s)?:\/\/discord.com\/channels\/[0-9]{17,}\/[0-9]{17,}\/[0-9]{17,}/)
        if (!matched) return

        const enable = await client.database.settings.get<ISettingsDB>(msg.guildID!)
